...
def update_tag_checkboxes(self):
    """
    更新标签展示区域，使用 QListWidget 支持双击编辑和拖动排序
    """
    # 清空现有的标签
    ...
    if self.current_image_name:
        ...
        for tag in tags:
            ...
            item.setFlags(item.flags() | Qt.ItemIsDragEnabled)  # 可拖动
            ...
            # 应用全局字体
            item.setFont(GLOBAL_FONT)
            item.setSizeHint(QSize(100, 35))  # 设置项的大小

            self.tags_list.addItem(item)
            self.tag_checkboxes.append(item)
