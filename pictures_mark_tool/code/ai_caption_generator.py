...
def generate_caption_for_selected(self):
    ...
    for i, image_name in enumerate(self.selected_images):
        ...
        try:
            # 生成提示词
            caption = self.image_processor.generate_caption_with_ai(image_name, self.ai_prompt_input.toPlainText())
...
def generate_caption_with_ai(self, image_name, prompt_text=None):
    """ 使用AI为指定图片生成提示词 """
    try:
        # 获取图片路径
